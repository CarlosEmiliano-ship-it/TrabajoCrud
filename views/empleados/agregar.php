    ...
    <main class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-4">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div class="bg-zinc-200 shadow-lg p-4 w-full rounded-lg">
                ...
                <p class="text-center py-2">Formulario para agregar un nuevo empleado</p>
                <form action="<?= BASE_URL ?>controllers/controllerEmpleados.php" method="POST" class="flex flex-col gap-4 max-w-sm mx-auto" enctype="multipart/form-data">
                    <input type="text" name="nombres" placeholder="Nombres del empleado" class="border border-gray-300 rounded-md p-2" required>
                    <input type="text" name="apellidos" placeholder="Apellidos del empleado" class="border border-gray-300 rounded-md p-2" required>
                    <input type="date" name="fecha_nac" class="border border-gray-300 rounded-md p-2" required>
                    <input type="number" step="0.01" name="salario" placeholder="Salario del empleado" class="border border-gray-300 rounded-md p-2" required>
                    <input type="text" name="puesto" placeholder="Puesto del empleado" class="border border-gray-300 rounded-md p-2" required>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[700px] overflow-y-auto pr-1">
                        <?php foreach ($empleados as $emp): ?>
                            <div class="bg-white rounded-xl shadow flex flex-col overflow-hidden">
                                <img src="<?= BASE_URL . htmlspecialchars(!empty($emp['imagen']) ? $emp['imagen'] : 'assets/img/default.png') ?>"
                                ...
                                <div class="flex gap-2 p-3 pt-0">
                                    <a href="<?= BASE_URL ?>controllers/editarEmpleado.php?id=<?= (int)$emp['id'] ?>"
