...
if (!$estudiante) {
    header("Location: " . BASE_URL . "views/estudiantes/agregar.php?status=error&msg=estudiante_no_encontrado");
    exit;
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <main class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-4">
        <div class="bg-zinc-200 shadow-lg p-4 max-w-lg mx-auto rounded-lg">
            <h2 class="uppercase text-xl font-bold text-center py-4">Editar Estudiante</h2>
            ...
            <form action="actualizarEstudiante.php" method="POST" class="flex flex-col gap-4">
                ...
                <input type="text" name="nombres" value="<?= htmlspecialchars($estudiante['nombres']) ?>"
                       placeholder="Nombres" class="border border-gray-300 rounded-md p-2" required>

                <input type="text" name="apellidos" value="<?= htmlspecialchars($estudiante['apellidos']) ?>"
                       placeholder="Apellidos" class="border border-gray-300 rounded-md p-2" required>
                ...
                <div class="flex gap-2">
                    ...
                    <a href="<?= BASE_URL ?>views/estudiantes/agregar.php" class="flex-1 bg-gray-400 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-xl text-center">
                        Cancelar
                    </a>
                </div>
            </form>
